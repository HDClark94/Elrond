...
def plot_speed_histogram_with_error_all_trials2(processed_position_data, output_path, track_length):
    ...
    plt.xlim(0,track_length)
    published.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_track_plot(ax, track_length)
    published.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_vr_plot(ax)
    ax.set_ylim(0)
...
def plot_firing_rate_maps_hmt(spike_data, processed_position_data, output_path, track_length, trial_type=0):
    ...
    for cluster_index, cluster_id in enumerate(spike_data.cluster_id):
        ...
        if len(firing_times_cluster)>1:
            ...
            plt.xlabel('Location (cm)', fontsize=18, labelpad = 10)
            plt.xlim(0,200)
            published.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_track_plot(ax, 200)
            published.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_vr_plot(ax)
            ax.set_ylim(0)
            plt.locator_params(axis = 'x', nbins  = 3)
            plt.locator_params(axis = 'y', nbins  = 4)
