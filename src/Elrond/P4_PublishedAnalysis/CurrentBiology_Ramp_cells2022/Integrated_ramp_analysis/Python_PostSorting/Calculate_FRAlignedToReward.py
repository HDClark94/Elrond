...
import Published_analysis.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.Split_DataByReward
...
def plot_rewarded_spikes_on_track(recording_folder, spike_data, rewarded):
    ...
    for index, row in spike_data.iterrows():
        ...
        trial_numbers = np.asarray(row["trial_number"].iloc[0])
        trial_types = np.asarray(row["trial_type"].iloc[0])
        x_position_cm = np.asarray(row["x_position_cm"].iloc[0])
        rewarded_trials = np.asarray(row["rewarded_trials"].iloc[0])

        if rewarded:
            suffix="_rewarded"
            trial_types = trial_types[np.isin(trial_numbers, rewarded_trials)]
            x_position_cm = x_position_cm[np.isin(trial_numbers, rewarded_trials)]
            trial_numbers = trial_numbers[np.isin(trial_numbers, rewarded_trials)]

        # not using non beaconed trials
        trial_numbers = trial_numbers[trial_types != 1]
        x_position_cm = x_position_cm[trial_types != 1]
        trial_types = trial_types[trial_types != 1]

        trial_numbers = compress_trial_numbers(trial_numbers)
        spikes_on_track = plt.figure(figsize=(3.7,3))
        ax = spikes_on_track.add_subplot(1, 1, 1)  # specify (nrows, ncols, axnum)
        ax.plot(x_position_cm[trial_types==0], trial_numbers[trial_types==0], '|', color='Black', markersize=2.5)
        #ax.plot(x_position_cm[trial_types==1], trial_numbers[trial_types==1], '|', color='Blue', markersize=2.5)
        ax.plot(x_position_cm[trial_types==2], trial_numbers[trial_types==2], '|', color='Blue', markersize=2.5)

        if max(trial_numbers)<60: # make the lines slightly longer when the trial nmuber is low to avoid empty space
            ax.plot(x_position_cm[trial_types==0], trial_numbers[trial_types==0]+0.5, '|', color='Black', markersize=2.5)
            ax.plot(x_position_cm[trial_types==0], trial_numbers[trial_types==0]-0.5, '|', color='Black', markersize=2.5)

            #ax.plot(x_position_cm[trial_types==1], trial_numbers[trial_types==1]+0.5, '|', color='Blue', markersize=2.5)
            #ax.plot(x_position_cm[trial_types==1], trial_numbers[trial_types==1]-0.5, '|', color='Blue', markersize=2.5)

            ax.plot(x_position_cm[trial_types==2], trial_numbers[trial_types==2]+0.5, '|', color='Blue', markersize=2.5)
            ax.plot(x_position_cm[trial_types==2], trial_numbers[trial_types==2]-0.5, '|', color='Blue', markersize=2.5)

        plt.ylabel('Trials', fontsize=18, labelpad = 0)
        plt.xlabel('Location (cm)', fontsize=18, labelpad = 10)
        plt.xlim(0,200)
        Published_analysis.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_track_plot(ax, 200)
        Published_analysis.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.plot_utility.style_vr_plot(ax)
        plt.ylim(0, max(trial_numbers))
        plt.locator_params(axis = 'y', nbins  = 4)
        plt.locator_params(axis = 'x', nbins  = 3)
        ax.set_xticklabels(['-30', '70', '170'])
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + session_id + '_track_firing_Cluster_' + str(cluster_id) + '_normal'+suffix+'.png', dpi=200)
        plt.close()



def compress_trial_numbers(trial_numbers):
    # this function takes in trial numbers [1,1,1,1,3,3,3,5,5,5] associated with single spikes and
    # compresses the array down to incremental values [1,1,1,1,2,2,2,3,3,3]
    trial_jumps = np.append(np.diff(trial_numbers), 0)
    trial_jumps[trial_jumps>=1] = 1
    compressed_trial_numbers = np.cumsum(trial_jumps)+1
    return compressed_trial_numbers

def plot_rewarded_spikes_on_track_beaconed(recording_folder,spike_data, rewarded):
    print('plotting spike rasters for rewarded trials...')
    save_path = recording_folder + '/Figures/spike_trajectories_with_reward'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    ...
    for index, row in spike_data.iterrows():
        ...
        # filter out all beaconed trials
        trial_numbers = trial_numbers[trial_types==0]
        x_position_cm = x_position_cm[trial_types==0]
        trial_types = trial_types[trial_types==0]
        ...
        ax.set_xticklabels(['-30', '70', '170'])
        plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
        plt.savefig(save_path + '/' + session_id + '_track_firing_Cluster_' + str(cluster_id) + '_normal_beaconed'+suffix+'.png', dpi=200)
        plt.close()
...
def plot_rewarded_spikes_on_track_with_tt(recording_folder,spike_data):
    ...
    for cluster in range(len(spike_data)):
        ...
        # split spikes by reward
        #spike_position_cm, spike_trial_numbers = split_spikes_by_reward(spike_data, cluster, rewarded_trials)
        rewarded_beaconed_position_cm, rewarded_nonbeaconed_position_cm, rewarded_probe_position_cm, rewarded_beaconed_trial_numbers, rewarded_nonbeaconed_trial_numbers, rewarded_probe_trial_numbers = Published_analysis.CurrentBiology_Ramp_cells2022.Integrated_ramp_analysis.Python_PostSorting.Split_DataByReward.split_trials_by_reward(spike_data, cluster)
