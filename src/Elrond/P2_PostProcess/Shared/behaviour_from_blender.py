...
def generate_position_data_from_blender_file(recording_path, processed_path):
    ...
    session_name = get_session_names([recording_path])[0]
    ...
    if session_name == 'dvd_waitscreen': 
        names += ["dvd_x_position", "dvd_y_position", "something"]


    blender_data = pd.read_csv(blender_files[0], skiprows=4, sep=";",names=names)        
    blender_data.dropna()
    ...
    if session_name == 'dvd_waitscreen':
        position_data["dvd_x_position"] = blender_data["dvd_x_position"]
        position_data["dvd_y_position"] = blender_data["dvd_y_position"]
        position_data["something"] = blender_data["something"]
