...
def calculate_border_score(firing_fields, bin_size_cm):
    ...
    if len(firing_fields) > 0:
        normalised_distance_mat = distance_matrix_border(firing_fields[0], bin_size_cm)
...
def find_neighborhood(masked_rate_map, rate_map, firing_rate_of_max, threshold):
    ...
    for bin_to_test in firing_field_bins:
        ...
        neighbors = find_neighbors(bin_to_test, max_x=(masked_rate_map.shape[0] - 1),
                                   max_y=(masked_rate_map.shape[1] - 1))
...
def test_if_field_is_small_enough(field_indices, rate_map):
    ...
    if number_of_pixels_in_field > 3 * (
            number_of_pixels_on_map / 4):  # if field is bigger than 3/4 of map, reject field
        return False
    else:
        return True
