...
local_path = Published_analysis.NatComms_LocalGridHD_2020.folder_path_settings.get_local_path() + '/pattern_of_directions_all_cells/'
...
def load_field_data(output_path, server_path, spike_sorter, animal, df_path='/DataFrames'):
    ...
    for recording_folder in glob.glob(server_path + '*'):
        ...
        if os.path.exists(data_frame_path):
            ...
            # spatial_firing = P3_PostProcess.compare_first_and_second_half.analyse_first_and_second_halves(prm, position, spatial_firing)
...
def save_corr_coef_in_csv(good_grid_coef, good_grid_cells_p, file_name):
    ...
    correlation_data.to_csv(
        Published_analysis.NatComms_LocalGridHD_2020.folder_path_settings.get_local_path() + '/field_histogram_shapes/' + file_name + '.csv')
...
def shuffle_field_data(field_data, path, number_of_bins, number_of_times_to_shuffle=1000, shuffle_type='occupancy'):
    ...
    for index, field in field_data.iterrows():
        ...
        shuffle_indices = Published_analysis.NatComms_LocalGridHD_2020.shuffle_field_analysis.get_random_indices_for_shuffle(field, number_of_times_to_shuffle, shuffle_type=shuffle_type)
        shuffled_hd_field = []
...
def add_shuffled_hd_histograms(fields):
    ...
    field_df = Published_analysis.NatComms_LocalGridHD_2020.shuffle_field_analysis.add_rate_map_values_to_field_df_session(spatial_firing, fields)
