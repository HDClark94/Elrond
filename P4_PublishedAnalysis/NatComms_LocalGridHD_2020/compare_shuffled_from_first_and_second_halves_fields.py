...
import numpy as np
import os
import Published_analysis.NatComms_LocalGridHD_2020.compare_shuffled_from_first_and_second_halves
...
local_path = Published_analysis.NatComms_LocalGridHD_2020.folder_path_settings.get_local_path() + '/compare_first_and_second_shuffled_fields/'
...
def get_half_rate_map_from_whole_cell(spatial_firing_all, session_id, cluster_id):
    ...
    first_half, second_half, position_first, position_second = Published_analysis.NatComms_LocalGridHD_2020.compare_shuffled_from_first_and_second_halves.split_in_two(cell)
...
def distributive_shuffle(field_data, number_of_bins=20, number_of_times_to_shuffle=1000):
    ...
    for index, field in field_data.iterrows():
        ...
        shuffle_indices = Published_analysis.NatComms_LocalGridHD_2020.shuffle_field_analysis.get_random_indices_for_shuffle(field, number_of_times_to_shuffle, shuffle_type='distributive')
        shuffled_hd_field = []
...
def compare_observed_and_shuffled_correlations(iterator, grid_data, all_cells, aggregated_data, sampling_rate_video):
    ...
    first_half = Published_analysis.NatComms_LocalGridHD_2020.shuffle_field_analysis.analyze_shuffled_data(first_half, local_path + '/first/',
    ...
    second_half = Published_analysis.NatComms_LocalGridHD_2020.shuffle_field_analysis.analyze_shuffled_data(second_half, local_path + '/second/',
                                                                                                            sampling_rate_video,
                                                                                                            number_of_bins=20,
                                                                                                            shuffle_type='distributive')
...
def process_data(server_path, spike_sorter='/MountainSort', df_path='/DataFrames', sampling_rate_video=30, tag='mouse'):
    if tag == 'mouse':
        ...
        all_cells = pd.read_pickle(
            Published_analysis.NatComms_LocalGridHD_2020.folder_path_settings.get_local_path() + '/compare_first_and_second_shuffled/all_mice_df.pkl')
    else:  # rat data
        accepted_fields = pd.read_excel(local_path + 'included_fields_detector2_sargolini.xlsx')
        all_cells = pd.read_pickle(
            Published_analysis.NatComms_LocalGridHD_2020.folder_path_settings.get_local_path() + '/compare_first_and_second_shuffled/all_rats_df.pkl')


    all_data = pd.read_pickle(local_path + 'all_' + tag + '_df.pkl')
    all_data = add_cell_types_to_data_frame(all_data)
    if tag == 'mouse':
        all_data = Published_analysis.NatComms_LocalGridHD_2020.shuffle_field_analysis_all_animals.tag_accepted_fields_mouse(all_data, accepted_fields)
    elif tag == 'rat':
        all_data = Published_analysis.NatComms_LocalGridHD_2020.shuffle_field_analysis_all_animals.tag_accepted_fields_rat(all_data, accepted_fields)
